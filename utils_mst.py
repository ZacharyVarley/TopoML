...
__author__ = "Miloš Stanojević" __license__ = "MIT"

Stanojević, Miloš, and Shay B. Cohen. "A root of a problem: Optimizing
single-root dependency parsing." Proceedings of the 2021 Conference on Empirical
Methods in Natural Language Processing. 2021.


Here we leverage the fact that B_0 is equal to one more than the number of edges
in the MST above threshold 2 * alpha.
...
class EdgePriorityQueue:
    ...
    def extract_max(self) -> Tuple[int, int, float]:
...
def tarjan(weights: np.ndarray) -> np.ndarray:
    ...
    vertices_queues = [EdgePriorityQueue(dep, weights[:, dep]) for dep in range(n)] + [
    ...
    while len(vertices_queues[a]) != 0:
        ...
        if vertices_in[u] is None:
            ...
            for i in vertices_children[c]:
                vertices_parent[i] = c
                vertices_highway[vertices_highway == vertices_highway[i]] = (
                    c  # union by collapsing
                )
...
def parents_to_edges(parents: np.array) -> np.array:
    """
    Convert parent index representation to edge pairs.

    Args:
        parents: Tensor of shape (n_nodes,) where parents[i] contains parent index of node i,
                with -1 indicating root

    Returns:
        Tensor of shape (n_edges, 2) containing edge pairs
    """
    children = np.where(parents != -1)[0]
    parents = parents[children]
    return np.stack([parents, children], axis=1)


def get_mst_edges(weights: torch.Tensor, one_root: bool = True) -> torch.Tensor:
    """
    Compute minimum spanning tree from edge weights matrix.

    Args:
        weights: Tensor of shape (n_nodes, n_nodes) where weights[head][dep] is weight of head->dep edge
        one_root: Whether to enforce a single root node

    Returns:
        Tensor of shape (n_nodes - 1, 2) containing edge pairs
    """
    neg_weights_np = (-1.0 * weights).cpu().numpy()
    edges = parents_to_edges(fast_parse(neg_weights_np, one_root))
    return torch.from_numpy(edges).to(weights.device)
